<?php
$connection = mysqli_connect("localhost", "root", "", "ObatKu");
...
function add_order()
{
    if (isset($_GET['order'])) {
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            // Calculate total order price for each item
            $totalOrderPrice = $item[0]['item_price'] * $quantity;

            // Insert order for the current item
            $query = "INSERT INTO orders (user_id, item_id, order_quantity, order_date, order_total) 
                      VALUES ('$user_id','$item_id','$quantity','$date', '$totalOrderPrice')";
